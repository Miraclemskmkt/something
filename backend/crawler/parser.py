...
EVENT_FORMAT_ONLINE = re.compile(
    r"线上(?:举办|进行|开展|开营|夏令营|活动|形式|方式)|"
    r"在线(?:举办|进行|开展)|网络(?:远程)?(?:举办|进行|开营)|"
    r"远程(?:举办|开营|进行)|腾讯会议|zoom|以线上"
)
EVENT_FORMAT_OFFLINE = re.compile(
    r"线下(?:举办|进行|开展|开营|夏令营|活动|形式|方式)|"
    ...
    for date_str in (publish_date, deadline):
        if date_str and len(date_str) >= 4:
            try:
                ...
                pass
    if not years:
        # 官网通知栏标题常不含年份，含夏令营/预推免等关键词则视为当前招生季
...
def compact_spaced_text(text: str) -> str:
    ...
    for kw in ("开营", "报名", "截止", "申请", "举办", "活动", "填报", "发布"):
...
def extract_registration_period(text: str) -> tuple[str | None, str | None]:
    ...
    for m in period_re.finditer(text):
        ...
        if any(k in window for k in ("开营", "夏令营", "入营", "活动")) and not any(
            k in window for k in ("报名", "登录", "系统", "yzbm", "填报")
        ):
            continue
...
def _clean_event_time(raw: str) -> str:
    ...
    if len(raw) > 50 and not re.search(r"\d{1,2}[月\-/.]|至|到|\d{4}", raw):
        return ""
    if not re.search(r"\d{1,2}[月\-/.]|至|到|\d{4}-\d{2}-\d{2}|\d{4}年", raw):
        return ""
...
def extract_all_times(text: str, html: str | None = None, url: str | None = None) -> dict[str, str | None]:
    ...
    return {
        "publish_date": resolve_publish_date(focused, html, url),
        "deadline": extract_registration_deadline(focused),
        "event_time": extract_event_time(focused),
    }
...
def apply_extracted_times(
    ...
    times = extract_all_times(text, html, item.url)
    if times["publish_date"]:
        item.publish_date = times["publish_date"]
    if times["deadline"]:
        item.deadline = times["deadline"]
    if times["event_time"]:
...
def extract_page_text(html: str) -> str:
    ...
    for sel in CONTENT_SELECTORS:
        ...
        if el:
            ...
            if len(text) >= 80:
                return _trim_boilerplate(text[:12000])
    ...
    if body:
        ...
        if body_text:
            ...
            if best_text and len(best_text) >= 120:
                return _trim_boilerplate(best_text[:12000])
            return body_text
    return _trim_boilerplate(soup.get_text(" ", strip=True)[:12000])
...
def enrich_from_html(item: ParsedAnnouncement, html: str) -> ParsedAnnouncement:
    ...
    if not text:
        if item.url:
            ...
            if url_pub and not item.publish_date:
                item.publish_date = url_pub
        return item
    if not item.summary or len(item.summary or "") < 80:
        item.summary = text[:500]

    apply_extracted_times(item, text, html)

    fmt = extract_event_format(focus_notice_text(text))
    if fmt:
...
def parse_wechat_search_results(
    ...
    *,
    ...
    for box in soup.select("ul.news-list li, div.txt-box, div.news-box"):
        title_el = box.select_one("h3 a, h4 a, a[target='_blank']")
        if not title_el:
            continue
